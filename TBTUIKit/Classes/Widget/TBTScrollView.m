...
@interface TBTScrollView ()<TBTRefreshHeaderDelegate,LoadMoreFooterDelegate>
{
    BOOL _refreshing;
    BOOL _loadingMore;
    BOOL _hasMore;
...
@property (nonatomic, weak)id<TBTScrollViewDelegate> realDelegate;

@end

@implementation TBTScrollView
...
- (void)setDelegate:(id<UICollectionViewDelegate>)delegate {
    self.realDelegate = (id<TBTScrollViewDelegate>)delegate;
...
- (void)egoRefreshTableHeaderDidTriggerRefresh:(TBTRefreshHeaderView*)view {
    if ([_realDelegate respondsToSelector:@selector(scrollViewDidTriggerRefresh:)]) {
        _refreshing = [_realDelegate scrollViewDidTriggerRefresh:self];
    }
...
- (NSDate *)egoRefreshTableHeaderDataSourceLastUpdated:(TBTRefreshHeaderView*)vie {
    return [NSDate date];
}
...
- (void)loadMoreTableFooterDidTriggerRefresh:(TBTLoadMoreFooterView *)view {
    if (_hasMore && [_realDelegate respondsToSelector:@selector(scrollViewDidTriggerLoadmore:)]) {
        _loadingMore = [_realDelegate scrollViewDidTriggerLoadmore:self];
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (_enablePullLoadingMore && self.autoLoadNextPageDistance > 0 && !_loadingMore && _hasMore) {
        ...
        if (yMargin>-1*self.autoLoadNextPageDistance && yMargin<0 && offset.y > 0) {
            if ([_realDelegate respondsToSelector:@selector(scrollViewDidTriggerLoadmore:)]) {
                _loadingMore = [_realDelegate scrollViewDidTriggerLoadmore:self];
                if (_loadingMore) {
